...
public class TPPResultsParser {
	 ...
	 * @throws Exception
	 */
	public Map<IProphetReportedPeptide, Collection<IProphetResult>> getResultsFromAnalysis( TPPAnalysis analysis ) throws Exception {
					...
					for( SearchHit searchHit : searchResult.getSearchHit() ) {
						for( AnalysisResult analysisResult : searchHit.getAnalysisResult() ) {
							if( analysisResult.getAnalysis().equals( "interprophet" ) ) {
								 ...
								 * and kojak_tpp only score a single rank 1 hit for a spectrum. If we only keep the peptide that
								 * kojak_tpp scored, we may lose valuable information if the leucine->isoleucine variant of that
	 ...
	 * @throws Exception
	 */
	private Collection<IProphetReportedPeptide>  getAllLeucineIsoleucineSubstitutions( IProphetReportedPeptide reportedPeptide, SearchResult searchResult, TPPAnalysis analysis ) throws Exception {
	 ...
	 * @throws Exception
	 */
	private IProphetReportedPeptide getReportedPeptide( SearchHit searchHit, TPPAnalysis analysis ) throws Exception {
	 ...
	 * @throws Exception
	 */
	private IProphetReportedPeptide getCrosslinkReportedPeptide( SearchHit searchHit, TPPAnalysis analysis ) throws Exception {
	 ...
	 * @throws Exception
	 */
	private IProphetReportedPeptide getLooplinkReportedPeptide( SearchHit searchHit, TPPAnalysis analysis ) throws Exception {
	 ...
	 * @throws Exception
	 */
	private IProphetReportedPeptide getUnlinkedReportedPeptide( SearchHit searchHit, TPPAnalysis analysis ) throws Exception {
	 ...
	 * @throws Exception
	 */
	private IProphetPeptide getPeptideFromSearchHit( SearchHit searchHit, TPPAnalysis analysis ) throws Exception {
	 ...
	 * @throws Exception
	 */
	private IProphetPeptide getPeptideFromLinkedPeptide( LinkedPeptide linkedPeptide, TPPAnalysis analysis ) throws Exception {
	...
	private IProphetResult getResult( MsmsRunSummary runSummary, SpectrumQuery spectrumQuery, SearchHit searchHit ) throws Exception {
		...
		// get the scores for peptideprophet and kojak_tpp
		for( AnalysisResult analysisResult : searchHit.getAnalysisResult() ) {
		...
		if( result.getInterProphetScore() == null )
			throw new Exception( "Missing kojak_tpp score for result: " + spectrumQuery.getSpectrum() );
